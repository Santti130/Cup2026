export function renderCountdown() {
    ...
    // Fondo según tamaño de pantalla (responsivo manual)
    if (window.innerWidth >= 768) {
        section.style.backgroundImage = "url('/img/fondo-home-countdown.png')"
    } else {
        section.style.backgroundImage = "url('/img/fondo-home-countdown-phone.png')"
    }
    section.style.minHeight = '100vh'
    
    // -------------------------------------------------------------------------
    // 2. ESTRUCTURA HTML (MÓVIL + DESKTOP)
    // -------------------------------------------------------------------------
    section.innerHTML = /*html*/`

    <!-- ==========================================
    VERSIÓN MÓVIL — visible solo en móvil
    =========================================== -->
    <div class="md:hidden flex flex-col items-center gap-6 w-full px-6 pt-5 pb-28 z-10">
        ...
        <div class="relative w-52 h-52 flex items-center justify-center">
            <!-- Capa difuminada — se extiende un poco fuera del anillo -->
        ...
        <div class="w-full max-w-sm">
            <div class="flex items-center justify-between mb-2">
                <span class="text-white/60 font-bebas text-sm tracking-widest uppercase">Progreso del evento</span>
                <span id="progress-percent-mobile" class="text-white font-bebas text-sm">0%</span>
            </div>
            <div class="w-full bg-white/10 rounded-full h-2">
                <div id="progress-bar-mobile" class="h-2 rounded-full bg-gradient-to-r from-yellow-400 via-green-400 to-emerald-500" style="width:0%"></div>
            </div>
        </div> -->

        <!-- Subtítulo -->
        <div class="flex flex-col items-center gap-1">
            <span class="text-xl font-black text-white text-center font-bebas tracking-wider uppercase bg-clip-text text-transparent bg-gradient-to-b from-white to-white/60 drop-shadow-[0_2px_10px_rgba(255,255,255,0.3)]">PARA EL INICIO DE LA COPA DEL MUNDO 2026</span>
        </div>

        <!-- Separador -->
        <div class="w-full max-w-sm border-t border-white/10"></div>

        <!-- Próximos partidos título -->
    ...
    (TODO: pegar aquí el código desktop actual)-->
    ...
    // -------------------------------------------------------------------------
    // 3. LÓGICA DEL COUNTDOWN (actualización en tiempo real)
    // -------------------------------------------------------------------------
    
    // Fechas base para el cálculo del progreso
    const startDate    = new Date('2025-12-01T00:00:00Z')
    const worldCupDate = new Date('2026-06-11T20:00:00Z')

    /**
     * Actualiza todos los elementos del DOM relacionados con el countdown:
     * - Días, horas, minutos, segundos (móvil y desktop)
     * - Barras de progreso (desktop y móvil)
    ...
    function updateCountdown() {
        ...
        // --- Desktop ---
        const d = section.querySelector('#days')
        if (d) d.textContent = String(days).padStart(2, '0')
        const h = section.querySelector('#hours')
        if (h) h.textContent = String(hours).padStart(2, '0')
        const m = section.querySelector('#minutes')
        if (m) m.textContent = String(minutes).padStart(2, '0')
        const s = section.querySelector('#seconds')
        if (s) s.textContent = String(seconds).padStart(2, '0')

        // --- Móvil ---
        const dm = section.querySelector('#days-mobile')
        if (dm) dm.textContent = String(days).padStart(2, '0')
        const hm = section.querySelector('#hours-mobile')
        if (hm) hm.textContent = String(hours).padStart(2, '0')
        const mm = section.querySelector('#minutes-mobile')
        if (mm) mm.textContent = String(minutes).padStart(2, '0')
        const sm = section.querySelector('#seconds-mobile')
        if (sm) sm.textContent = String(seconds).padStart(2, '0')
        ...
        const pp = section.querySelector('#progress-percent')
        if (pp) pp.textContent = percent + '%'

        // Barra de progreso móvil (comentada en HTML, pero se actualiza por si se activa)
        const pbm = section.querySelector('#progress-bar-mobile')
        if (pbm) pbm.style.width = percent + '%'
        const ppm = section.querySelector('#progress-percent-mobile')
        if (ppm) ppm.textContent = percent + '%'

        // Anillo SVG de progreso (móvil)
    ...
    // -------------------------------------------------------------------------
    // 4. RENDERIZADO DE PRÓXIMOS PARTIDOS (datos mock)
    // -------------------------------------------------------------------------
    ...
    // --- Tarjetas para móvil (vista simple) ---
    const containerMobile = section.querySelector('#cards-container-mobile')
    if (containerMobile) {
    ...
    // --- Tarjetas para desktop (flip cards con efecto 3D) ---
    ...
    // -------------------------------------------------------------------------
    // 5. RETORNO DEL NODO PARA SU INSERCIÓN EN EL DOM
    // -------------------------------------------------------------------------
    return section
}